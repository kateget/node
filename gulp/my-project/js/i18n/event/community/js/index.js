$(function(){
	var swiper = new Swiper('.swiper-container1', {
	    ...
	    spaceBetween: 10,
	    loop:true,
	    // loopAdditionalSlides:3,//防止swiper出现断篇
	...
	// Rank	TEAM	Team Leader	R1 KILL PTS	R1 RANK PTS	R1 TOTAL	R2 KILL PTS	R2 RANK PTS	R2 TOTAL	R3KILL PTS	R3 RANK PTS	R3 TOTAL	 TOTAL KILL PTS	TOTAL RANK PTS	TOTAL PTS
	var indonesiaData = [
	...
	$(document).on("click",".rank_more li",function(){
		var $this = $(this);
	...
	$(document).on("click",".start_players .play_icon,.start_players .swiper-icon",function(e){
		var $this = $(this);
		var index = $this.parents(".swiper-slide").index();
		if($this.hasClass("swiper-icon")){
			$this.siblings(".play_icon").trigger("touchstart");
		}
		else{
    ...
    window.onYouTubeIframeAPIReady = function() {
        player = new YT.Player('video_dialog', {
            ...
            events: {
            	onReady : function(){
                    $(document).on("click",".dialog_container .dialog_X",function(){
    ...
    window.onresize=function(){  
         if(document.documentElement.clientWidth <= 850){
        	window.resizeTo(850,9999);
        	window.resizeBy(850,9999);
        	window.focus();
         }else{
